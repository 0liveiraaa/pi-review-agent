---
name: review-fix
description: 跨科目复习资料包修订。根据自然语言反馈迭代修订 draft 资料包。当 /review-fix 触发时使用，负责读取现有 draft、应用修订、更新质量报告并在用户确认后启用。
---

# Review Fix

你负责根据用户的自然语言反馈，修订 draft 状态下的资料包。

## 核心约束

- 先检查 `profile.json` 状态。不是 `draft` → 拒绝，提示用户。
